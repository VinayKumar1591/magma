  ...
  if (bytes > 0) {
    ...
    if ((state != REGISTERED_CONNECTED) &&
      ...
      initial_context_setup_request(as_msg->ue_id, amf_ctx, as_msg->nas_msg);
       ...
       // (msg->nas_info == AMF_AS_NAS_INFO_SR)) {
    // amf_app_handle_nas_dl_req(as_msg->ue_id, as_msg->nas_msg,
    // M5G_AS_SUCCESS);
    // }
...
static int amf_as_establish_rej(
  ...
  switch (msg->nas_info) {
    case AMF_AS_NAS_INFO_REGISTERED:
      break;
    case AMF_AS_NAS_INFO_SR:
      size = amf_service_rejectmsg(msg, &nas_msg);
...
static int amf_service_rejectmsg(
  ...
  nas_msg->security_protected.plain.amf.msg.service_reject.sec_header_type
      .sec_hdr                         = SECURITY_HEADER_TYPE_NOT_PROTECTED;
  ...
  if (msg->pdu_session_status_ie & AMF_AS_PDU_SESSION_STATUS) {
    nas_msg->plain.amf.msg.service_reject.pdu_session_status.iei =
        PDU_SESSION_STATUS;
    nas_msg->plain.amf.msg.service_reject.pdu_session_status.len = 0x02;
    nas_msg->plain.amf.msg.service_reject.pdu_session_status.pduSessionStatus =
        msg->pdu_session_status;
  }

  nas_msg->plain.amf.msg.service_reject.extended_protocol_discriminator
      .extended_proto_discriminator = M5G_MOBILITY_MANAGEMENT_MESSAGES;
  nas_msg->plain.amf.msg.service_reject.message_type.msg_type =
      M5G_SERVICE_REJECT;
  nas_msg->plain.amf.msg.service_reject.sec_header_type.sec_hdr =
      SECURITY_HEADER_TYPE_NOT_PROTECTED;

  nas_msg->plain.amf.msg.service_reject.cause.iei         = M5GMM_CAUSE;
  nas_msg->plain.amf.msg.service_reject.cause.m5gmm_cause = msg->amf_cause;

  nas_msg->plain.amf.msg.service_reject.t3346Value.iei        = GPRS_TIMER2;
  nas_msg->plain.amf.msg.service_reject.t3346Value.len        = 1;
  nas_msg->plain.amf.msg.service_reject.t3346Value.timervalue = 60;

  nas_msg->header.message_type = M5G_SERVICE_REJECT;
  size += NAS5G_MESSAGE_CONTAINER_MAXIMUM_LENGTH;
  OAILOG_FUNC_RETURN(LOG_NAS_AMF, size);
}
}  // namespace magma5g
